...
def select_model(args, device, map_location):
...
def get_infor():
    ...
    sys.argv = [
        'Pred_Depth.py',
        '--task', 'depth',
        '--img_path', '/home/liex/Desktop/merge/MealsRetrieval-ROOMELSA/Depth/image',  # image dir
        '--output_path', '/home/liex/Desktop/merge/MealsRetrieval-ROOMELSA/Depth/output' # output dir
    ]
...
if __name__ == '_main_':
    args, root_dir = get_infor() # change to your dir
    # create output folder, create map, set devide(cuda or cpu)
    os.system(f"mkdir -p {args.output_path}")
    map_location = (lambda storage, loc: storage.cuda()) if torch.cuda.is_available() else torch.device('cpu')
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    #load model
    trans_totensor, model, trans_rgb = select_model(args)
    trans_topil = transforms.ToPILImage()

    # load images, create output
    img_path = Path(args.img_path)
    if img_path.is_file():
        save_outputs(args.img_path, os.path.splitext(os.path.basename(args.img_path), trans_totensor, model, trans_rgb, device, args)[0])
    elif img_path.is_dir():
        for f in glob.glob(args.img_path+'/*'):
            save_outputs(f, os.path.splitext(os.path.basename(f), trans_totensor, model, trans_rgb, device, args)[0])
    else:
